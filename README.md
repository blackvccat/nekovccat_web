...
**属于本仓库自己的那部分**（合并之后在 MARCUS 这条线上做的，对照分支里没有）：Harness 架构与运行时管理、明暗两套主题、夜泊动态壁纸、CRT 扫描线动态化、聊天额度分档与防洪、访客名单接 MySQL / PostgreSQL、数据库连接教程。
...
**对照分支本身不在这份代码里**：它是仓库之外的另一份完整副本（带它自己的文档），本仓库不依赖它、单独就能跑。所以在这里找不到 `nekovccat_web-main v1.1/` 之类的目录——那是作者本机的布局，不是这个项目的依赖。
