...
本仓库是 **MARCUS** 那条线。另一条线是**对照分支 v1.1（NEKO / My World）**，两份代码同源，所以能逐文件对照。本项目的做法是**按清单逐项采纳**：每条写成 BACKLOG 里的稳定 ID（`A*` Agent 能力 / `S*` 安全缺口），带改动文件、依赖与验证方式；**不接受的也写下来**（伴侣彩蛋线、队列式并发器…），免得以后重复讨论。
