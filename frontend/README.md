...
# DATABASE_URL="postgresql://postgres:changeme@localhost:5432/marcus_app?schema=public"
```
...
│   │       └── route.ts
│   ├── layout.tsx               # 根布局
│   ├── terminal/                # 唯一的页面：虚拟电脑桌面
│   │   ├── page.tsx
│   │   └── desktop.css          # 桌面样式与主题变量
│   ├── loading.tsx              # 全局加载状态
│   ├── error.tsx                # 全局错误边界
...
│
├── content/                      # 站点内容
│   ├── profile.ts               # 关于我、经历、项目、技能、邮箱
│   └── showcase.ts              # 长廊的画框与主站链接
│
...
│
├── public/                       # 静态资源
│   ├── images/                  # 桌面壁纸、头像与画廊缩略图
│   └── fonts/                   # Geist / Geist Mono / Jersey 25
│
...
站点只有一个页面：`app/terminal/page.tsx` 渲染的虚拟电脑桌面，线上地址是 `https://example.com/terminal`。域名根路径 `/` 属于主站，应用不接管它（本地 `http://127.0.0.1:3010/` 会是 404）。顶栏 HOME 指向主站 `https://example.com/`，Terminal 是当前桌面。

### 入口与旧地址

`next.config.ts` 的 `redirects()` 用真正的 HTTP 307 跳转：

| 来源 | 去向 |
| --- | --- |
| `/my-world` | `/terminal`（查询参数原样保留） |
| `/about` | `/terminal?app=explorer&tab=about` |
| `/contact` | `/terminal?app=explorer&tab=contact` |

`NEXT_PUBLIC_APP_URL` 是构建时变量，生产构建前应设为 `https://example.com/terminal`；它只用于 `metadataBase`，不影响路由。`SITE_ORIGIN`（同源校验）与 `CORS_ORIGINS` 必须只写 origin，不能带路径。

### 长廊

桌面里的 MARCUS Browser 有四个栏目，其中「长廊」的内容来自 `src/content/showcase.ts`：每条记录带编号、标题、简介、调性标注与本地缩略图，点击画框会在新标签页打开主站「共鸣」展厅里的对应作品（`https://example.com/player.html?...`），另有按钮直达 `https://example.com/index.html#/showcase`。

缩略图放在 `public/images/showcase/`（8 张 WebP）。站点的 CSP 是 `img-src 'self'`，所以画框只能使用本地图片，不能直接引用主站地址。
...
**注意**: 需要浏览器 API 的组件（桌面、播放器、画廊等）必须使用 `"use client"`。
...
# 创建数据库和用户
CREATE DATABASE marcus_app;
CREATE USER marcus_user WITH ENCRYPTED PASSWORD 'your_secure_password';
GRANT ALL PRIVILEGES ON DATABASE marcus_app TO marcus_user;
\q
```
...
```bash
# 克隆项目
git clone <your-repo-url> /var/www/marcus_app
cd /var/www/frontend/marcus_app
...
# 数据库配置
DATABASE_URL="postgresql://marcus_user:your_secure_password@localhost:5432/marcus_app?schema=public"
```
...
# 使用 PM2 启动应用
pm2 start npm --name "marcus-app" -- start
pm2 save
pm2 startup
```
...
services:
  app:
    ...
    ports:
      - "3000:3000"
    environment:
      - DATABASE_URL=postgresql://postgres:password@db:5432/marcus_app
      - NODE_ENV=production
    depends_on:
      - db
