...
export default function NekoBrowser({ initialPage = 'about', launch = 0, onSponsor }: { initialPage?: BrowserPage; launch?: number; onSponsor: () => void }) {
  ...
  return <div className="neko-browser">
    <div className="browser-menubar"><span>File</span><span>View</span><span>Bookmarks</span><span>NEKO Navigator / 01</span></div>
    <div className="browser-toolbar"><button className="pixel-button" aria-label="后退" disabled={index === 0} onClick={() => { setIndex(index - 1); setPhoto(null) }}>←</button><button className="pixel-button" aria-label="前进" disabled={index === history.length - 1} onClick={() => { setIndex(index + 1); setPhoto(null) }}>→</button><span className="browser-address inset-panel" aria-label="当前站内地址">neko://{selected}</span><span className="browser-online" title="本站内容">●</span></div>
    ...
    <div className="browser-document" ref={content}>
      ...
      {selected === 'minecraft' && <article><div className="browser-kicker">MINECRAFT / WORLD ARCHIVE</div><h1>在方块之间<span className="document-subtitle">一座城，一片山野，还有许多个慢慢搭建的日子。</span></h1><p>这里是我的 Minecraft 服务器影像档案。沿着街道走过广场、庭院与塔楼，把那些值得停下来的风景留在这里。</p><div className="gallery-filters" aria-label="建筑档案分类">{[['all', '全部'], ['city', '城市漫游'], ['garden', '山野与庭院'], ['architecture', '建筑手记']].map(([id, title]) => <button className="pixel-button" key={id} aria-pressed={category === id} onClick={() => { setCategory(id); setPhoto(null) }}>{title}</button>)}<small>{images.length} 帧</small></div>
        {activePhoto && <section className="gallery-viewer" aria-label="查看建筑大图"><button className="text-button" onClick={() => setPhoto(null)}>收起大图 ×</button><Image unoptimized src={activePhoto.src} width={activePhoto.width} height={activePhoto.height} alt={activePhoto.title} /><p>{activePhoto.title}</p></section>}
        <div className="minecraft-grid">{images.map(item => <button key={item.id} className="minecraft-photo" onClick={() => { setPhoto(item.id); content.current?.scrollTo({ top: 0, behavior: 'instant' }) }} aria-label={`查看 ${item.title}`}>
