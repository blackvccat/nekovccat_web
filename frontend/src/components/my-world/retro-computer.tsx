    ...
    <div className="agent-app">
      ...
      <div className="agent-messages inset-panel" ref={messagesContainer} role="log" aria-label="Agent 对话" aria-live="polite"
        ...
        <div className="agent-welcome">
          ...
          <div><div className="eyebrow">A LITTLE HELP, A LITTLE CURIOSITY.</div><h2>Hello, explorer<span className="pixel-cursor">_</span></h2><p>我是 NEKO，你的站内向导。想先去哪里看看？</p></div>
        </div>
      ...
      <form className="agent-composer" onSubmit={event => { event.preventDefault(); send() }}>
        <label className="sr-only" htmlFor="agent-message">给 NEKO Agent 发消息</label>
        <div className="composer-field inset-panel"><span aria-hidden="true">›</span><textarea id="agent-message" value={input} onChange={event => setInput(event.target.value)} placeholder="和 NEKO 聊聊…" rows={2} disabled={!isReady || isLoading} onKeyDown={event => {
