...
export default function SiteMusic() {
  ...
  return <div className={`site-music ${pathname === '/' ? 'music-over-scenery' : 'music-on-paper'}`} data-music-ui>
    {expanded && <section id="site-music-panel" className="site-music-panel" aria-label="NEKO 站内助手面板">
    ...
    <div className={`site-music-dock ${expanded ? 'is-open' : ''}`}>
      <button type="button" ref={trigger} className="site-music-launcher" aria-label={expanded ? '收起 NEKO 站内助手' : '打开 NEKO 站内助手'} aria-expanded={expanded} aria-controls="site-music-panel" onClick={() => { if (expanded) collapse(); else { setTab('agent'); setExpanded(true) } }}>
        <span className="site-music-symbol"><PixelIcon name="agent" size={32} /></span>
        <span className="site-music-label"><strong>NEKO <span>站内助手</span></strong><small>{current ? `音乐已载入 · ${favoriteName.trim() || current.label}` : '聊聊这个世界，顺便听首歌'}</small></span>
