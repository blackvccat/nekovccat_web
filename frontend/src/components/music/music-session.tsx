...
const STORAGE_KEY = 'marcus-music-library-v1'
...
type MusicSession = {
  ...
  error: string; notice: string; library: SavedMusic[]; ready: boolean; autoplay: boolean
  setInput: (input: string) => void; setFavoriteName: (name: string) => void
  load: (url: string, name?: string, autoplay?: boolean) => void; stop: () => void
...
export function MusicSessionProvider({ children }: { children: ReactNode }) {
  ...
  const load = (url: string, name?: string, playNow = false) => {
    try {
      ...
      setError('')
      // 网易云歌单不加载官方播放器，所以提示改成「去下面的曲目列表挑一首」。
      const listed = item.provider === 'netease' && item.type === 'playlist'
      setNotice(listed ? '这个歌单的曲目列在下面：点哪首就用官方播放器播哪首。' : playNow ? '正在用官方播放器播放。若没有自动开始，点播放器里的播放按钮。' : '使用播放器里的播放按钮开始听歌。站内切页会保留当前音乐。')
  ...
  return <MusicContext.Provider value={{ provider, input, current, favoriteName, error, notice, library, ready, autoplay,
    ...
    {children}
    <PersistentMusicPlayer current={current} slot={slot} autoplay={autoplay} />
...
function PersistentMusicPlayer({ current, slot, autoplay }: { current: MusicItem | null; slot: PlayerSlot | null; autoplay: boolean }) {
  ...
  // 网易云歌单由我们自己的曲目列表负责（点哪首播哪首），所以没有可见插槽时不挂官方播放器——
  // 它一次只列 10 首，加载出来反而多余。单曲与 Spotify 仍然常驻，保证切页、关窗不断音。
  const listedPlaylist = current?.provider === 'netease' && current.type === 'playlist'
  const mountFrame = !!current && (!listedPlaylist || visible)
  ...
  return <div className="persistent-music-player" hidden={!visible} data-testid="persistent-music-player" data-frame-loads={loadCount}
    ...
    {mountFrame && <iframe key={`${current.embedUrl}|${autoplay}`} src={autoplay ? current.autoplayUrl : current.embedUrl} title={`${MUSIC_PROVIDERS[current.provider].name}音乐播放器`}
    ...
    {visible && !slot!.active && <button type="button" className="music-player-activate" aria-label="激活 MARCUS Music 播放器" onClick={slot!.activate} />}
  </div>
}
