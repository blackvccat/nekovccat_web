...
const unlockAttempts = new ChatAttemptLimiter({ limit: 3, windowMs: 10000, cooldownMs: 30000 })

export function chatAttemptError(identity: string, scope: 'chat' | 'session' | 'unlock'): NextResponse | null {
  if (!guarded()) return null
  const result = (scope === 'chat' ? chatAttempts : scope === 'session' ? sessionAttempts : unlockAttempts).take(identity)
