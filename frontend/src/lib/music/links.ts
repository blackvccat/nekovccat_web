...
export type MusicItem = {
  ...
  url: string
  embedUrl: string
  /** 同一首歌的「点了就播」地址：只有用户主动点歌单条目时才会用到。 */
  autoplayUrl: string
  height: number
  label: string
}
...
function spotifyItem(type: keyof typeof SPOTIFY_TYPES, id: string): MusicItem {
  return {
    ...
    autoplayUrl: `https://open.spotify.com/embed/${type}/${id}?theme=0&autoplay=1`,
...
function neteaseItem(type: 'song' | 'playlist', id: string): MusicItem {
  ...
  return {
    ...
    autoplayUrl: `https://music.163.com/outchain/player?type=${isSong ? 2 : 0}&id=${id}&auto=1&height=${isSong ? 66 : 430}`,
