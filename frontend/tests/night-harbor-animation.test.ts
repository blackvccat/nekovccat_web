...
test('the atlas ships next to the poster with the same hashed name convention', () => {
  // 素材名是内容哈希：清单里的 image 必须指向本站自己的文件，否则组件会当成跨域素材拒绝加载。
  assert.match(manifest.image, /^\/images\/marcus-night-harbor\/ambience-[a-f0-9]{12}\.webp$/)
  assert.match(NIGHT_HARBOR_ASSETS.manifest, /^\/images\/marcus-night-harbor\/manifest-[a-f0-9]{12}\.json$/)
  for (const asset of [NIGHT_HARBOR_ASSETS.wallpaper, NIGHT_HARBOR_ASSETS.mobile, NIGHT_HARBOR_ASSETS.thumbnail]) {
    assert.match(asset, /^\/images\/marcus-night-harbor\/[a-z]+-[a-f0-9]{12}\.webp$/)
  }
})

test('every atlas frame stays inside the atlas image it will be drawn from', () => {
  // 组件在浏览器里做的也是这条检查；放在这里是为了在没有浏览器的情况下先失败一次。
  const atlas = readFileSync(new URL(`../public${manifest.image}`, import.meta.url))
  assert.ok(atlas.byteLength > 0)
  for (const region of manifest.regions) {
    for (const frame of region.frames) {
      assert.ok(frame.x + frame.width <= manifest.width, `${region.id} frame exceeds atlas width`)
      assert.ok(frame.y + frame.height <= manifest.height, `${region.id} frame exceeds atlas height`)
    }
  }
})
